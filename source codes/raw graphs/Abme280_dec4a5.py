...
for r in readings:
    if r['sensor_id'] == 210:
        ...
        if month == 12 and date == 5 or date == 4:
...
plt.title("BME280 Atmospheric Pressure Measurements Dec 4rd ~ Dec 5th")
...
plt.plot(pdates,sensor_210)

# Smoothed
plt.plot(pdates[:-40],moving_average(40, sensor_210),color='black',alpha = 0.7,label='smoothed by window 40')

# Mathematical model
y_octic = []
a,b,c,d,e,f,g,h,I = np.polyfit(timeindex, sensor_210, deg=8)
for i in timeindex:
